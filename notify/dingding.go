...
func HookTextDingGroupWithConf(text, token, secret string) {
	var receiver Robot
...
func HookSendMarkdown(title string, mkd string, token string, secret string) {
	...
	if len(mkd) > 0 {
		...
		receiver.secret = secret
		webHookUrl := receiver.Signature()
		params := receiver.SendMarkdown(title, mkd, []string{}, []string{}, false)
		SendRequest(webHookUrl, params)
	}
}
